...
void DynamixelController::initServer()
{
  ...
  motor_ = node_handle_.advertiseService("motor_run", &DynamixelController::motor_on, this);
  sensor_ = node_handle_.advertiseService("sensor_move", &DynamixelController::sensormove_on, this);
  click_ = node_handle_.advertiseService("click_run", &DynamixelController::click_on, this);
}
...
bool DynamixelController::motor_on(robot_msgs::motor_srv::Request &req, robot_msgs::motor_srv::Response &res){
  bool result = false;
  const char* log=NULL;
  ...
  double time = req.delay;
  int dir = req.direction;
  ...
  if(dir == 1){      //right
    ...
    value[2] = 0;
    value[3] = 0;}  
  result = dxl_wb_->syncWrite(SYNC_WRITE_HANDLER_FOR_GOAL_VELOCITY, id, 4, value, 1, &log);
...
bool DynamixelController::sensormove_on(robot_msgs::sensor_move::Request &req, robot_msgs::sensor_move::Response &res){
  bool result = false;
  bool goal_position = false;
  const char* log=NULL;
  ...
  //result = dxl_wb_->itemWrite(id, item_name.c_str(), value, &log);
  result = dxl_wb_->syncWrite(SYNC_WRITE_HANDLER_FOR_GOAL_POSITION, id, 2, value, 1, &log);
...
bool DynamixelController::click_on(robot_msgs::click::Request &req, robot_msgs::click::Response &res){
  bool result;
  const char* log=NULL;
...
int main(int argc, char **argv)
{
  ...
  if (argc < 2)
  {
    ROS_ERROR("Please set '-port_name' and  '-baud_rateclick_on' arguments for connected Dynamixels");
    return 0;
  }
  else
  {
